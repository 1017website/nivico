...
<div class="mn-inner">
  ...
  <a class="mn-item {{ request()->routeIs('login', 'account.*') ? 'on' : '' }}" href="{{ auth()->check() ? (auth()->user()->isAdmin() ? route('admin.dashboard') : route('account.dashboard')) : route('login') }}"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><span>Akun</span></a>
</div>
</div>
