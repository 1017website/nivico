...
<div class="pc{{ $isSoldOut ? ' soldout' : '' }}" onclick="location.href='{{ $url }}'">
    ...
    <div class="pc-img">
        <img src="{{ $product->image ?: asset('images/placeholder-product.svg') }}" alt="{{ $product->name }}" loading="lazy" onerror="this.onerror=null;this.src='{{ asset('images/placeholder-product.svg') }}'">
        @if($isSoldOut)
            <div class="pc-stock-empty">Stok Kosong</div>
        @endif
    </div>
    <div class="pc-body">
        ...
        @if($isSoldOut)
            <button class="btn-kj" type="button" disabled onclick="event.stopPropagation()">
                Stok Kosong
            </button>
