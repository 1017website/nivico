    ...
    <tbody>
      @foreach($messages as $m)
        <tr style="{{ $m->is_read ? '' : 'background:#f0f7ff' }}">
          <td>
            <div style="font-weight:600"><a href="{{ route('admin.messages.show', $m) }}" style="color:inherit">{{ $m->name }}</a> @unless($m->is_read)<span class="badge b-paid" style="margin-left:4px">Baru</span>@endunless</div>
          ...
          <td style="white-space:nowrap">
            <a class="btn btn-sm btn-blue" href="{{ route('admin.messages.show', $m) }}"><i class="fa-solid fa-eye"></i> Buka</a>
            @unless($m->is_read)
