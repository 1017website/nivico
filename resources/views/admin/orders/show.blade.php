      ...
      <div class="table-wrap"><table>
        ...
        <tbody>
          @foreach($order->items as $it)
            <tr>
              <td style="display:flex;align-items:center;gap:10px">@if($it->image)<img class="thumb" src="{{ $it->image }}">@endif<div>{{ $it->product_name }}<div style="font-size:11.5px;color:var(--muted)">{{ $it->sku }}</div></div></td>
